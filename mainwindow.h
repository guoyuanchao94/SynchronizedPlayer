...
private slots:
    ...
    void setVideoSource(QString url);   //设置网络视频源
...
private:
    ...
    QAudioSink *udpAudioSink = nullptr; //发送数据到音频输出设备
    QAudioSource *udpAudioSource = nullptr; //从音频输入设备捕获音频数据
    QIODevice *udpAudioSinkDevice = nullptr;    //数据源
    ...
    QString serverURL = "";
    QSet<QString> setFilePath;  //存储打开的文件夹的路径
    QListWidget *playList = nullptr;    //播放列表
    QPushButton *btnDisconnect = nullptr;   //客户端断开连接按钮
private:
